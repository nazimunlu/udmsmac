...
const BusinessExpensesView = () => {
    ...
    useEffect(() => {
        const fetchExpenses = async () => {
            const { data, error } = await supabase.from('transactions').select('*').eq('type', 'expense-business');
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-xl text-gray-800">Logged Business Expenses</h3>
            ...
            <TransactionFormModal isOpen={isTransactionModalOpen} onClose={() => setIsTransactionModalOpen(false)} defaultCategory="business" />
        </div>
    );
};

export default BusinessExpensesView;